...
def Loss_Vout(V, Pout, Vout, gradient=False):
    ...
    if gradient:
        ...
        dLoss =  0 * V
    # dLoss = tf.math.scalar_mul(tf.constant(100), dLoss) # scale Vout dLoss
    # dLoss *= 100000
    # Loss_norm *= 100000
...
def Loss_Vin(V, Pin, Vin, bound, gradient=False):
    ...
    Loss = tf.linalg.matmul(V, tf.transpose(Pin), b_is_sparse=True) - tf.cast(tf.multiply(Vin, 50.0), tf.float32) #*50
...
def Loss_Vpos(V, parameter, gradient=False):
    ...
    # Loss = ReLU(-V)
    # dLoss = ∂(ReLU(-V)^2/∂V
    Loss = tf.keras.activations.relu(-V) # parameter.LB
...
def get_V0(inputs, parameter, targets, trainable, verbose=False):
    ...
    if targets.shape[0] > 0: # Initialize AMN when targets provided
        ...
        # Vin  = inputs
        Vin = tf.cast(tf.multiply(inputs, 50.0), tf.float32)
...
def train_model(parameter, Xtrain, Ytrain, Xtest, Ytest, verbose=False):
    ...
    Niter = 1 # maximum number of attempts to fit - was 5, now to 1
...
def train_evaluate_model(parameter, verbose=False):
    ...
    # kfold = KFold(n_splits=param.xfold, shuffle=False)
    kiter = 0
